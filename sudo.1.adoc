...
*sudo* (-i | -s) [-k] [-n] [-u <__user__>] [<__command__> [--] [<__args__>...]] +
*sudo* [-ikns] [-u <__user__>] <__command__> [--] [<__args__>...] +
*sudo* -k +
*sudo* [-h]
...
It supports only a subset of the {sudo} options (both short and long variants) that have an equivalent in {doas}, plus options *-i* (*--login*) and *-k* (*--reset-timestamp*).
...
*-k*, *--reset-timestamp*::
Invalidate the user's cached credentials by executing {doas} with the *-L* option.
When used without a command, it clears the cached credentials and exits.
+
*WARNING:* When used with a command, this option is only approximated.
Unlike {sudo}, OpenDoas cannot ignore the cached credential for a single invocation while also preventing the timestamp from being refreshed.
This shim calls `doas -L` before and after the command instead, which is not atomic and may leave or expose a refreshed timestamp if interrupted or used concurrently!
